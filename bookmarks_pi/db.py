...
def _migrate(conn: sqlite3.Connection) -> None:
    ...
    if "rotation" not in note_columns:
    ...
    if "width" not in note_columns:
        conn.execute("ALTER TABLE sticky_notes ADD COLUMN width INTEGER")
    if "height" not in note_columns:
        conn.execute("ALTER TABLE sticky_notes ADD COLUMN height INTEGER")
...
def update_sticky_note_rotation(conn: sqlite3.Connection, owner_username: str, note_id: int, rotation: int) -> bool:
    cursor = conn.execute(
        "UPDATE sticky_notes SET rotation = ? WHERE id = ? AND owner_username = ?",
        (rotation, note_id, owner_username),
    )
    conn.commit()
    return cursor.rowcount > 0


def update_sticky_note_color(conn: sqlite3.Connection, owner_username: str, note_id: int, color: str) -> bool:
    cursor = conn.execute(
...
def update_sticky_note_size(
    conn: sqlite3.Connection, owner_username: str, note_id: int, width: int, height: int
) -> bool:
    cursor = conn.execute(
        "UPDATE sticky_notes SET width = ?, height = ? WHERE id = ? AND owner_username = ?",
        (width, height, note_id, owner_username),
    )
    conn.commit()
    return cursor.rowcount > 0
